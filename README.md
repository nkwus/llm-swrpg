...
A locally-hosted AI chatbot that answers questions about Star Wars tabletop RPG rulebooks. It uses **Retrieval-Augmented Generation (RAG)**: your PDF rulebooks are indexed into a local vector database, and a locally-running language model generates answers using only the text found in those PDFs.

Everything runs on your machine. No data leaves your network. No API keys are required.
...
- [Adding Your PDFs and Populating the Database](#adding-your-pdfs-and-populating-the-database)
...
3. BUILD a prompt
   (inject the retrieved chunks as context into a structured prompt)
      ...
      ▼
4. CALL Ollama
   (send the prompt to the local LLM, which generates an answer
...
Before the chatbot can answer anything, you must run `populate_database.py` once for each set of PDFs. This does the following:
...
├── llm.py                 # Prompt construction and Ollama API call (LLM interface)
├── rag.py                 # answer_question() — orchestrates retrieval → LLM
│
├── server.py              # FastAPI web server — routes only, no business logic
├── populate_database.py   # One-time ingestion script — reads PDFs, fills ChromaDB
├── query_chat.py          # Terminal chat interface
│
├── static/
│   └── index.html         # Single-page chat UI (plain HTML/CSS/JS, no framework)
│
├── data/                  # Place your PDF rulebooks here (not committed to git)
├── chroma/                # ChromaDB vector database (generated, not committed to git)
│
├── .env                   # Local configuration overrides (not committed to git)
...
- **`llm.py`** — if you swap Ollama for an external API (e.g. OpenAI, Groq), you only change this file.
- **`rag.py`** — the orchestration logic. Calls retrieval and llm but knows nothing about HTTP or the terminal.
- **`server.py`** — HTTP concerns only. Knows nothing about how answers are produced.
...
Ollama runs language models locally. It exposes a local HTTP API on port 11434 that this app calls.
...
Then pull the model used by this app (Mistral 7B, approximately 4 GB download):
...
The defaults in `.env` are sensible starting values. See [Configuration](#configuration) for what each setting does.
...
| `EMBEDDING_MODEL` | `all-MiniLM-L6-v2` | The sentence-transformers model used to create embeddings. Must be the same model for both ingestion and querying. **Do not change this after ingestion without re-running `populate_database.py`.** |
| `OLLAMA_URL` | `http://localhost:11434` | The base URL of the Ollama server. Change this if Ollama is running on a different machine or port. |
| `OLLAMA_MODEL` | `mistral` | The Ollama model to use for generating answers. The model must already be pulled (`ollama pull <model>`). |
...
> **Important:** The `EMBEDDING_MODEL` must be identical between the ingestion run and the query-time run. If you change the embedding model, you must delete the `chroma/` directory and re-run `populate_database.py`.

---

## Adding Your PDFs and Populating the Database

This step is required before the chatbot can answer any questions. You only need to repeat it when you add, remove, or update PDF files.

### 1. Start Ollama

Ollama must be running as a background service. On most Linux systems after installation it runs automatically. Check:

```bash
ollama list
```

If you get a connection error, start it manually:

```bash
ollama serve
```

Or via systemd if it was installed that way:
...
Copy your Star Wars RPG PDF files into the `data/` directory:

```bash
cp "/path/to/Edge of the Empire Core Rulebook.pdf" data/
cp "/path/to/Age of Rebellion.pdf" data/
```

Any `.pdf` files in `data/` will be processed. The directory is in `.gitignore` so your PDFs will not be committed to the repository.
...
- Load the embedding model (first run downloads it, ~90 MB)
- Read every PDF in `data/`
- Split the text into chunks
- Create embeddings for each chunk
- Store everything in `chroma/`

For a typical rulebook (400–600 pages), this takes 2–10 minutes depending on your hardware.

**Sample output:**

```text
Loading embedding model: all-MiniLM-L6-v2
Initializing ChromaDB at: chroma
Reading PDF: Edge of the Empire Core Rulebook.pdf
Created 3842 chunks from Edge of the Empire Core Rulebook.pdf
Done! ChromaDB is populated.
```

### 4. Re-running ingestion

The script uses `get_or_create_collection`, so running it again will append new data rather than replacing it. If you want a clean rebuild (e.g. after changing the embedding model or chunk settings), delete the `chroma/` directory first:
...
```bash
ollama list   # if this works, Ollama is running
```
...
The first request after startup will be slow (a few seconds) because the embedding model and ChromaDB collection are loaded into memory at startup. Subsequent requests will be fast on the retrieval side; the LLM generation time depends on your hardware.
...
2. Start the server: `uv run uvicorn server:app --reload`
...
Restart the server. No re-ingestion required — the embedding model is independent of the LLM.

### Checking what models are available locally
...
### "Error: could not reach the local LLM. Is Ollama running?"

Ollama is not running or is not reachable on the configured URL.

1. Check if it is running: `ollama list`
2. If not, start it: `ollama serve`
3. Confirm `OLLAMA_URL` in `.env` matches where Ollama is actually running (default: `http://localhost:11434`)
...
Run ingestion: `uv run python populate_database.py`
...
2. Check that the relevant rulebook is in `data/` and was ingested
3. Some questions require information spread across many pages — this is a fundamental RAG limitation (see [Known Limitations](#known-limitations))
...
On first run, the `sentence-transformers` library downloads the embedding model (~90 MB). This is cached for subsequent runs. The Ollama model is also loaded into memory on first use — this is normal.
...
Very long answers, or questions that need many chunks to answer accurately, may cause the LLM to lose track of earlier context. If you are using a small model (3B–7B parameters), reducing `RETRIEVAL_K` may paradoxically improve answer quality by keeping the prompt shorter and more focused.

---

## Extending the App

### Swapping the LLM provider (e.g. to use OpenAI or Groq instead of Ollama)

All LLM logic is isolated in `llm.py`. To switch to an OpenAI-compatible API:

1. Add the `openai` package: `uv add openai`
2. Replace `call_ollama()` in `llm.py` with a function that calls the OpenAI client
3. Add `OPENAI_API_KEY` to `.env` and read it in `config.py`
...
Add a new route to `server.py` following the same pattern as the existing `/api/chat` route. If the endpoint needs database access, it is available via `app.state.embedder`, `app.state.collection`, and `app.state.settings`.

---

## Testing

The test suite covers all business logic using mocks — no Ollama instance, no ChromaDB database, and no embedding model downloads are required to run tests. Tests are fast and fully self-contained.
...
├── test_llm.py        # Prompt building, Ollama API call, error handling
...
**`test_llm.py`** — verifies that `build_prompt()` includes the question, all context chunks, and the system instruction. For `call_ollama()`, it verifies the correct URL and model are used, the response text is returned and whitespace-stripped, and that connection errors, timeouts, and missing response keys all return a readable error string rather than raising an exception.
...
**`test_server.py`** — verifies the HTTP layer using FastAPI's `TestClient`. Covers: the `GET /` route returns HTML containing the chat UI, `POST /api/chat` returns the answer from `answer_question()`, question whitespace is stripped, an empty question returns a prompt message without calling the LLM, and malformed requests return HTTP 422.

### Approach: mocking external dependencies

All tests mock the boundaries of the unit under test:

- `call_ollama()` tests mock `requests.post` — the HTTP call never leaves the process
- `retrieve_relevant_chunks()` tests pass a `MagicMock` collection — no ChromaDB on disk is needed
- `answer_question()` tests mock `retrieve_relevant_chunks` and `call_ollama` — they test orchestration logic only
...
| `requests` | ≥2.33 | Makes HTTP calls to the Ollama API |
